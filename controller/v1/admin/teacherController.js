...
export const addTeacher = async (req, res, next) => {
  try {
    ...
    if (!phoneRegex.test(phone)) {
      return next(
        new httpError("Phone number must be a 10-digit number.", 408)
      );
    }
    ...
    if (teacher) {
      return next(
        new httpError("teacher with this email or phone already exists!", 400)
      );
    }
    ...
    const getNewTeacher = await Teacher.findById(newTeacher._id)
    ...
    res
      .status(201)
      .json({ message: "teacher created successfully", data: getNewTeacher });
  } catch (error) {
    console.log(error);
...
export const findTeacher = async (req, res, next) => {
  try {
    ...
    const getTeacher = await Teacher.findById(teacher._id)
      .select("-__v -is_deleted -createdAt -updatedAt -password")
      .populate({
        path: "subject",
        select: "name",
      });
...
export const updateTeacherDetailes = async (req, res, next) => {
  try {
    ...
    const getTeacher = await Teacher.findById(updateTeacher._id)
...
export const deleteManyTeachers = async (req, res, next) => {
  try {
    const { ids } = req.body;
    ...
    res.status(202).json({
      message: `Successfully deleted ${updateResult.modifiedCount} teacher.`,
    });
  } catch (error) {
    console.log("err", error);
    return next(new httpError("Error deleting teacher", 500));
  }
};
