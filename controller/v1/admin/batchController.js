...
export const addBatch = async ( req, res, next ) => {
    try {
      ...
      const getBatch = await Batch.findById(newBatch._id).select('-__v -createdAt -updatedAt -is_deleted')
      res.status(201).json({
        status:true,
  ...
  export const listBatch = async( req, res, next) =>{
    ...
    try {
        ...
        const allBatches = await Batch.find(filter)
          ...
          .skip(skip)
          .limit(limit)
          .select('-password -is_deleted -__v -createdAt -updatedAt').populate({
            path: 'in_charge',
...
export const findBatch = async( req, res, next) =>{
    ...
    try {
        ...
        res.status(200).json( { message:`batch founded successfully` , data : getBatch} )
...
export const deleteBatch = async ( req, res, next)=>{
    try {
        ...
        if ( !deleteOneBatch ) {
            ...
            res.status(202).json({ message:`batch deleted successfully` })
